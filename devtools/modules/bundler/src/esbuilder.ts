...
import { fstat, stat, statSync } from "node:fs";
  ...
  public async executeBuilds(): Promise<void> {
    ...
    for (const variant of this.variants) {
      for (const format of this.formats) {
        ...
        try {
          await build(config);
          console.log(`   → ${ outfile ? basename(outfile).padEnd(basename(outfile).split(".")[0].length + 21 + 2) : dirname(outdir as string) } ✅${label} ${outfile ? `${(statSync(outfile as string).size / 1024).toFixed(1)}Kb` : ""}`);
        } catch (err) {
          console.error(`❌ Build failed: ${outfile || outdir};\n`, err);
          throw err;
        }
      }
    }
