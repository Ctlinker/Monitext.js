 ...
 ** Note: non-equality isn't synonym of non-assignability
 */
export type NotEqual<T, U> = (<G>() => G extends T ? 1 : 2) extends
	<G>() => G extends U ? 1 : 2 ? false
	: true;
...
export type Equals<T extends any[]> = T extends [infer First, ...infer Rest]
	? Rest extends [] ? true // only one element → trivially equal
	: Rest extends [infer Second, ...infer More]
		? Equal<First, Second> extends true ? Equals<[Second, ...More]> // recurse
		: false
	: true
	: true;
 ...
 ** Note: non-equality isn't synonym of non-assignability
 */
export type NotEquals<T extends any[]> = T extends [infer First, ...infer Rest]
	? Rest extends [] ? true // only one element → trivially equal
	: Rest extends [infer Second, ...infer More]
		? NotEqual<First, Second> extends true ? NotEquals<[Second, ...More]> // recurse
		: false
	: true
	: true;
