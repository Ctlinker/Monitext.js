...
> = Source extends { required: readonly string[] }
	? {
			...
			[K in Exclude<keyof Props, Source['required'][number]>]?: HandleSchema<
				Props[K]
			>;
		} & {
			// required props
			[K in Extract<keyof Props, Source['required'][number]>]: HandleSchema<
				Props[K]
			>;
		}
	: {
...
export type HandleObject<S> = S extends {
	type: 'object';
	properties: infer P extends Record<string, TSchema>;
}
...
type HandleItems<S> = S extends { items: infer X extends TSchema[] }
	? {
			[K in keyof X]: HandleSchema<X[K]>;
		}
	: S extends { items: infer X extends boolean }
		? X extends true
			? any[]
			: never[]
		: any[];
...
export type HandleArray<S> = S extends { type: 'array' }
	? S extends { prefixItems: infer P extends readonly TSchema[] }
		? S extends { items: infer I extends TSchema | boolean }
			? MergePush<
					{
						[K in keyof P]: HandleSchema<P[K extends keyof P ? K : never]>;
					},
					I extends boolean
						? I extends true
							? any[]
...
export type HandleOneOf<S> = S extends {
	oneOf: infer X extends readonly TSchema[];
}
	? HandleSchema<X[number]> // map each schema in the array to its handled type and union them
	: never;
 ...
 */
export type HandleEnum<S> = S extends { enum: readonly (infer X)[] }
	? X
	: never;
 ...
 */
export type HandleSchema<S> = S extends { type: 'string' }
	? HandleString<S>
