...
export type HandleObject<S> = S extends
    { type: "object"; properties: infer P extends Record<string, TSchema> }
...
export type HandleOneOf<S> = S extends
    { oneOf: infer X extends readonly TSchema[] } ? HandleSchema<X[number]> // map each schema in the array to its handled type and union them
    : never;
